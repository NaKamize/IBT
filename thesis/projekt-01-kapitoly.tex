...
inicializácia dočastného poľa s $poz\_S$\;
    \While{zásobník nie je prázdny} {
        ...
        \uElseIf{$ X \in N $}{
            variácia := získaj\_variáciu(var\_poz)\;
            $r := (X_1, X_2, \dots, X_n) \rightarrow (x_1, x_2, \dots. x_n) \in$ LL--tabuľke[X, t]\;
            $pravidlo := r[vari\acute{a}cia]$\;
            odkaz na symbol z dočasného poľa vo vnútri zásobníka je pos\;
            \eIf{$deep\_push$}{
                $do\check{c}as\_pole.remove(pos)$\;
                vymeň v zasobniku pos za $oto\check{c}(pravidlo[2][1])$\;
                $deep\_push = False$\;
            }{
                $do\check{c}as\_pole.remove(pop(X))$\;
                vymeň X za $oto\check{c}(pravidlo[2][0])$\;
